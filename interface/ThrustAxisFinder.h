...
template <class EventClass> class ThrustAxisFinder : public BaseOperator<EventClass> {
    ...
    virtual bool process( EventClass & ev ) {
      ...
      auto neg_thrust_f = [&reco_jets](double t_phi) {
        ...
        for (auto & j : reco_jets ) {
          ...
          neg_thrust -= j.Pt()*std::abs(
                        std::cos(ROOT::Math::VectorUtil::Phi_mpi_pi(j.Phi() - t_phi))
                        );
        }
        return neg_thrust;
      };
